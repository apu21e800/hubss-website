...
export const metadata = buildMetadata({
  title: "Request a Printed Catalogue",
  description:
    "Have the printed HUB Surface Systems catalogue mailed to your office - every decorative pavement system, application and specification, in the book our specifiers keep on the shelf.",
  slug: "request-catalogue",
});
          ...
          <aside className="lg:pt-14">
            {cover && (
              <div className="overflow-hidden rounded-2xl" style={{ border: "1px solid var(--border-color)", background: "var(--bg-card-neutral)" }}>
                {/* A plain img: catalogue rasters are already the exact sizes
                    they are shown at and must not route through /_next/image. */}
                <img
                  src={cover}
                  alt={`HUB Surface Systems Catalogue ${catalogue.edition ?? ""} cover`}
            ...
            <div className="mt-5 text-sm" style={{ color: "var(--text-secondary)" }}>
              <p className="mb-3">
                <strong style={{ color: "var(--text-primary)" }}>{catalogueTotal} pages.</strong> Decorative pavement
                solutions, coast to coast.
              </p>
              {catalogueReady && (
                <p className="mb-2">
                  <Link href="/catalogue" className="font-semibold underline" style={{ color: "var(--accent-text)" }}>
                    Read it online now
                  </Link>{" "}
              ...
              {catalogue.download && (
                <p>
                  <a href={catalogue.download.href} download className="font-semibold underline" style={{ color: "var(--accent-text)" }}>
                    Download the PDF
                  </a>{" "}
